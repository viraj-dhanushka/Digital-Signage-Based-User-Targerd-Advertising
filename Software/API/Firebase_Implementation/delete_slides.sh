...
# delete assets
curl -s -k -X DELETE "${API_URL}/female_25_32" -H "${H_ACCEPT}" -H "${H_TYPE}" -d "$(female_25_32)"; # -k or --insecure 
curl -s -k -X DELETE "${API_URL}/generic_ads" -H "${H_ACCEPT}" -H "${H_TYPE}" -d "$(generic_ads)"; # -k or --insecure 
curl -s -k -X DELETE "${API_URL}/male_25_32" -H "${H_ACCEPT}" -H "${H_TYPE}" -d "$(male_25_32)"; # -k or --insecure 
